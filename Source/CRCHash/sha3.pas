...
{.$define USE_MMXCODE  }   {Use MMX for Keccak permutation}
...
(*************************************************************************
 ...
 REMARKS         :  1. For 32-bit compilers with int64 (FPC, D6+) there are
                    ...
                    2. The current implementation needs little-endian machines and
                       supports only LSB bit sized messages for msglen <> 0 mod 8
...
(*-------------------------------------------------------------------------
 (C) Copyright 2012-2015 Wolfgang Ehrhardt
  ...
  {$else}
    {$ifndef FPC}
      {$ifndef CONDITIONALEXPRESSIONS}
        {$undef USE_MMXCODE} {Delphi 5 or lower}
      {$endif}
    {$endif}
  {$endif}
  {$ifdef USE_64BITCODE}
