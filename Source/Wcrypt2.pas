...
type
  ...
  CRYPT_TIME_STAMP_REQUEST_INFO = record
    ...
    {$ELSE}
    pszTimeStampAlgorithm :PAnsiChar;   // pszObjId
    pszContentType        :PAnsiChar;   // pszObjId
    {$ENDIF}
...
//--------------------------------------------------------------------------
  ...
  {$ELSE}
  X509_CERT                        = (PAnsiChar(1));
  X509_CERT_TO_BE_SIGNED           = (PAnsiChar(2));
  X509_CERT_CRL_TO_BE_SIGNED       = (PAnsiChar(3));
  X509_CERT_REQUEST_TO_BE_SIGNED   = (PAnsiChar(4));
  X509_EXTENSIONS                  = (PAnsiChar(5));
  X509_NAME_VALUE                  = (PAnsiChar(6));
...
//--------------------------------------------------------------------------
  ...
  X509_AUTHORITY_KEY_ID            = (PAnsiChar(9));
  X509_KEY_ATTRIBUTES              = (PAnsiChar(10));
  X509_KEY_USAGE_RESTRICTION       = (PAnsiChar(11));
  X509_ALTERNATE_NAME              = (PAnsiChar(12));
  X509_BASIC_CONSTRAINTS          = (PAnsiChar(13));
  X509_KEY_USAGE                   = (PAnsiChar(14));
  X509_BASIC_CONSTRAINTS2          = (PAnsiChar(15));
  X509_CERT_POLICIES               = (PAnsiChar(16));
  {$ENDIF}
...
//--------------------------------------------------------------------------
  ...
  PKCS_UTC_TIME                    = (PAnsiChar(17));
  PKCS_TIME_REQUEST                = (PAnsiChar(18));
  RSA_CSP_PUBLICKEYBLOB            = (PAnsiChar(19));
  X509_UNICODE_NAME                = (PAnsiChar(20));

  X509_KEYGEN_REQUEST_TO_BE_SIGNED  = (PAnsiChar(21));
  PKCS_ATTRIBUTE                    = (PAnsiChar(22));
  PKCS_CONTENT_INFO_SEQUENCE_OF_ANY = (PAnsiChar(23));
  {$ENDIF}
...
//--------------------------------------------------------------------------
  ...
  {$ELSE}
  X509_UNICODE_NAME_VALUE    = (PAnsiChar(24));
  X509_ANY_STRING            = X509_NAME_VALUE;
  X509_UNICODE_ANY_STRING    = X509_UNICODE_NAME_VALUE;
  X509_OCTET_STRING          = (PAnsiChar(25));
  X509_BITS                  = (PAnsiChar(26));
  X509_INTEGER               = (PAnsiChar(27));
  X509_MULTI_BYTE_INTEGER    = (PAnsiChar(28));
  X509_ENUMERATED            = (PAnsiChar(29));
  X509_CHOICE_OF_TIME        = (PAnsiChar(30));
  {$ENDIF}
...
//  X509_AUTHORITY_INFO_ACCESS          (LPCSTR(32));
  ...
  {$ELSE}
  X509_AUTHORITY_KEY_ID2        = (PAnsiChar(31));
//  X509_AUTHORITY_INFO_ACCESS          (PAnsiChar(32));
  X509_CRL_REASON_CODE          = X509_ENUMERATED;
  PKCS_CONTENT_INFO             = (PAnsiChar(33));
  X509_SEQUENCE_OF_ANY          = (PAnsiChar(34));
  X509_CRL_DIST_POINTS          = (PAnsiChar(35));
  X509_ENHANCED_KEY_USAGE       = (PAnsiChar(36));
  PKCS_CTL                      = (PAnsiChar(37));

  X509_MULTI_BYTE_UINT          = (PAnsiChar(38));
  X509_DSS_PUBLICKEY            =  X509_MULTI_BYTE_UINT;
  X509_DSS_PARAMETERS           = (PAnsiChar(39));
  X509_DSS_SIGNATURE            = (PAnsiChar(40));
  PKCS_RC2_CBC_PARAMETERS       = (PAnsiChar(41));
  PKCS_SMIME_CAPABILITIES       = (PAnsiChar(42));
...
//--------------------------------------------------------------------------
  ...
  {$ELSE}
  PKCS7_SIGNER_INFO             = (PAnsiChar(500));
  {$ENDIF}
...
type
  ...
  CERT_ALT_NAME_ENTRY = record
    ...
    {$ELSE}
    {2}1: (pwszRfc822Name  :PWideChar);            //(encoded IA5)
    {3}2: (pwszDNSName     :PWideChar);            //(encoded IA5)
    {$ENDIF}
...
type
  ...
  CERT_POLICY_QUALIFIER_INFO = record
    ...
    {$ELSE}
    pszPolicyQualifierId  :PAnsiChar;            // pszObjId
    {$ENDIF}
...
type
  ...
  CERT_POLICY_INFO = record
    ...
    {$ELSE}
    pszPolicyIdentifier  :PAnsiChar;            // pszObjId
    {$ENDIF}
...
type
  ...
  CRYPT_CONTENT_INFO_SEQUENCE_OF_ANY = record
    {$IFDEF MSWINDOWS}
    pszObjId  :LPSTR;            // pszObjId
    {$ELSE}
    pszObjId  :PAnsiChar;            // pszObjId
    {$ENDIF}
    cValue   :DWORD;
...
type
  ...
  CRYPT_OID_FUNC_ENTRY = record
    {$IFDEF MSWINDOWS}
    pszOID  :LPCSTR;            // pszObjId
    {$ELSE}
    pszOID  :PAnsiChar;            // pszObjId
    {$ENDIF}
    pvFuncAddr :PVOID;
  end;
...
{$ELSE}
function CryptInitOIDFunctionSet(pszFuncName :PAnsiChar;
{$ENDIF}
...
function CryptRegisterOIDFunction(dwEncodingType :DWORD;
                                  ...
                                  {$ELSE}
                                  pszFuncName :PAnsiChar;
                                  pszOID :PAnsiChar; //OPTIONAL
                                  pwszDll :PWideChar; //OPTIONAL
                                  pszOverrideFuncName :PAnsiChar):BOOL ; stdcall;
                                  {$ENDIF}
...
function CryptRegisterDefaultOIDFunction(dwEncodingType :DWORD;
                                         ...
                                         pszFuncName :PAnsiCHar;
                                         dwIndex :DWORD;
                                         pwszDll :PWideChar):BOOL ; stdcall;
                                         {$ENDIF}
...
type
  PFN_CRYPT_ENUM_OID_FUNC = function (dwEncodingType :DWORD;
                                ...
                                const rgpwszValueName :array of LPCWSTR;
                                    ...
                                    cValue :DWORD;
                              const rgdwValueType :array of DWORD;
                              const rgpwszValueName :array of PWideChar;
                                    {$ENDIF}
...
function CryptEnumOIDFunction(dwEncodingType :DWORD;
                              ...
                              pszOID :LPCSTR;         //OPTIONAL
                              {$ELSE}
                              pszFuncName :PAnsiChar;    //OPTIONAL
                              pszOID :PAnsiChar;         //OPTIONAL
                              {$ENDIF}
                              dwFlags :DWORD;
                              pvArg :PVOID;
...
function CryptMsgOpenToEncode(dwMsgEncodingType :DWORD;
                              ...
                              {$ELSE}
                              pszInnerContentObjID :PAnsiChar;      //OPTIONAL
                              {$ENDIF}
...
const 
  ...
  CERT_STORE_PROV_FILE = (PAnsiChar(3));
  CERT_STORE_PROV_REG = (PAnsiChar(4));

  CERT_STORE_PROV_PKCS7 = (PAnsiChar(5));
  CERT_STORE_PROV_SERIALIZED = (PAnsiChar(6));
...
{$ELSE}
PFN_CERT_DLL_OPEN_STORE_PROV_FUNC = function (lpszStoreProvider :PAnsiChar;
{$ENDIF}
                                                dwEncodingType :DWORD;
                                                hCryptProv :HCRYPTPROV;
                                                dwFlags :DWORD;
...
function CryptSignAndEncodeCertificate(hCryptProv :HCRYPTPROV;
                                       ...
                                       {$ELSE}
                                       const lpszStructType :PAnsiChar; // "to be signed"
                                       {$ENDIF}
                                       pvStructInfo :PVOID;
                                       pSignatureAlgorithm :PCRYPT_ALGORITHM_IDENTIFIER;
                                 const pvHashAuxInfo :PVOID;
...
{$ELSE}
function CertAlgIdToOID(dwAlgId :DWORD):PAnsiChar ; stdcall;
{$ENDIF}
//+-------------------------------------------------------------------------
...
{$ELSE}
function CertOIDToAlgId(pszObjId :PAnsiCHar):DWORD ; stdcall;
{$ENDIF}
//+-------------------------------------------------------------------------
...
{$ELSE}
function CertFindExtension(pszObjId :PAnsiCHar;
{$ENDIF}
                           cExtensions :DWORD;
...
{$ELSE}
function CertFindAttribute(pszObjId :PAnsiCHar;
{$ENDIF}
                           cAttr :DWORD;
...
{$ELSE}
function CertFindRDNAttr(pszObjId :PAnsiChar;
{$ENDIF}
...
function CertStrToNameA(dwCertEncodingType :DWORD;
                        ...
                        {$ELSE}
                        var ppszError :array of PAnsiCHar):BOOL ; stdcall;  {--max-- iniziato qui}
                        {$ENDIF}
//+-------------------------------------------------------------------------
//--------------------------------------------------------------------------
...
{$ENDIF}
                        ...
                        {$ELSE}
                        var ppszError :array of PWideCHar):BOOL ; stdcall;
                        {$ENDIF}
...
{$ELSE}
function CertOpenSystemStoreA(hProv :HCRYPTPROV;
                              szSubsystemProtocol :PAnsiChar):HCERTSTORE ; stdcall;
...
function CertAddEncodedCertificateToSystemStoreA(szCertStoreName :PAnsiChar;
                                           const pbCertEncoded :PBYTE;
                                                cbCertEncoded :DWORD):BOOL ; stdcall;
...
function CertAddEncodedCertificateToSystemStore(szCertStoreName :PWideChar;
                                          const pbCertEncoded :PBYTE;
                                                cbCertEncoded :DWORD):BOOL ; stdcall;
{$ENDIF}
...
function FindCertsByIssuer(pCertChains :PCERT_CHAIN;
                           ...
                           pwszPurpose :PWideChar; // "ClientAuth" or "CodeSigning"
